export const bikes = [
  {
    id: 'hero-splendor',
    name: 'Hero Splendor Plus',
    tagline: 'Mileage King – Everyday reliability',
    image: '/images/hero_splendor.png',
     modelUrl: '/images/motorcycle+3d+model.glb',
     // Placeholder
    colors: [
      { name: 'Classic Black', hex: '#ffffff' },
      { name: 'Ruby Red', hex: '#cfb6b6ff' },
      { name: 'Ocean Blue', hex: '#1d4fb3' }
    ]
  },
   {
    id: 'royal-enfield-classic',
    name: 'Royal Enfield Classic 350',
    tagline: 'Retro legend with strong road presence',
    image: '/images/royal_enfield.png',
     modelUrl: '/images/modern+motorcycle+3d+model.glb',// Placeholder
    colors: [
      { name: 'Chrome Silver', hex: '#d1d1d1' },
      { name: 'British Red', hex: '#d37a7aff' },
      { name: 'Desert Sand', hex: '#edc9af' }
    ]
  },
  {
    id: 'yamaha-mt15',
    name: 'Yamaha MT 15 V2',
    tagline: 'Aggressive street style',
    image: '/images/yamaha_mt15.png',
    modelUrl: '/images/sport+motorcycle+3d+model.glb', // Placeholder
    colors: [
      { name: 'Dark Matte Blue', hex: '#ffffff' },
      { name: 'Cyan Storm', hex: '#675ec3ff' },
      { name: 'Night Fluo', hex: '#1f1c1cff' }
    ]
  },
  {
    id: 'honda-shine',
    name: 'Honda Shine 125',
    tagline: 'Smooth, reliable daily ride',
    image: '/images/hero_splendor.png',
    modelUrl: '/images/sport+motorcycle+3d+model.glb',
    colors: [
      { name: 'Black', hex: '#00000034' },
      { name: 'Grey', hex: '#808080' }
    ]
  },
  {
    id: 'bajaj-pulsar',
    name: 'Bajaj Pulsar 150',
    tagline: 'Sporty and youthful',
  image: '/images/yamaha_mt15.png',
    modelUrl: '/images/sport+motorcycle+3d+model.glb',
    colors: [
      { name: 'Neon Red', hex: '#ff0000' },
      { name: 'Neon Silver', hex: '#c0c0c0' }
    ]
  }
];
